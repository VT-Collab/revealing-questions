...
def load_data(method,user, data):
    filename = "../Data/Study2/user"+ user + "/plate_" + str(method) + "_" + data + ".pkl"
    result = pickle.load(open(filename, "rb"))
    return result

def main(user_n, method):
# def main():
...
#     method = sys.argv[2]
    ...
    """ loading the user study data """
    Questions = load_data(method, user_n, 'qs')
    Answers = load_data(method, user_n, 'ans')
    ...
    for idx in range(n_questions):
        ...
        """ here is where we import the questions asked from the human"""
        Qstar = Questions[idx]
        # print("Question: ",'\n',Qstar,'\n')
        """ here is where we import the questions asked from the human"""
        ...
        # xi_star = optimal_traj(questionset,Theta)
        # f_star = features(xi_star)
        # print(f_star)
        # print("\n")
