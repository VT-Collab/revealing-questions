...
def features(xi):
    ...
    return np.asarray([object, shelf, distance_to_target])
...
def demo(Q, duration, wait, sp, ep):
        playback = pld.TrajectoryClient(duration)
        playback.load_trajectory(Q[sp:ep])
        playback.send(wait)
...
def save_results(user, method, data, name):
    savename = "../Data/Study2/user" + user + "/shelf_" + method + "_"+ name + ".pkl"
...
def main():
    ...
    filename = "../Data/Questions/Q_shelf.pkl"
    ...
    for idx in range(n_questions):
        ...
        for i in range(2):
            ...
            demo(Qstar[i], duration_p, wait, 0, 3)
            grip.close_gripper()
            time.sleep(3)
            demo(Qstar[i], duration_d, wait, 3, 6)
            grip.open_gripper()
            time.sleep(1)
    ...
    # demo(best_traj, 4, wait, 0, 3)
    # grip.close_gripper()
    # time.sleep(3)
    # demo(best_traj, 2, wait, 3, 6)
    # grip.open_gripper()
    # time.sleep(1)
    ...
    save_results(user_n, method, Theta, 'th')
    save_results(user_n, method, Phi, 'ph')
    print('\n', "-----Data Saved-----")
